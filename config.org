...
Ensure that the non-standard (custom) themes which I use are installed so that they may be loaded in this configuration and activated as the current theme.
...
** NEXT octave mode (replacing matlab mode)
