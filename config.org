...
I also tend to switch themes between a light and dark theme and so I make it as easy as possible for myself to do so.
The ~elisp~ code here simply allows Emacs to consider custom themes to be safe and allow these themes to be loaded and activated as the current theme.
The cursor type is also modified to a *bar* not a /blinking box/.
...
#+begin_src emacs-lisp
  (use-package elpy
    :ensure t
    :init
    (setq pyvenv-activate "/home/jkoch/anaconda3/envs/main")
